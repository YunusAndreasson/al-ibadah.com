---
title: Vallfärd för pengar som man har fått från räntebaserade tillgångar
author: Muḥammad bin Sālih al-´Uthaymīn
source: Fatāwa Ibn ‘Uthaymīn, 21/105
categories:
  - Vallfärd (Hajj)
  - Övrigt
original_id: 893
description: 'All lovprisan tillhör Allāh. Får man pengar av någon som använder ränta är det inte fel att använda dessa pengar för att vallfärda.'
---

**Fråga:** Jag har inte utfört den obligatoriska vallfärden och någon gav mig pengar som jag skulle kunna vallfärda för men jag vet att personen använder ränta. Får jag vallfärda för dessa pengar?

**Svar:** All lovprisan tillhör Allāh. Får man pengar av någon som använder ränta är det inte fel att använda dessa pengar för att vallfärda. Man syndar heller inte om man accepterar det som man får då synden för räntan bärs av den som befattar sig med den. Den som tar emot detta på ett sätt som den islamiska lagen godkänner, som genom att få detta i form av en gåva, personen har inte syndat.

Beviset för detta är att sändebudet – över honom vare Allāhs frid och välsignelser – accepterade gåvor från judarna, han åt deras mat och han handlade med dem. Detta trots att det var känt att judarna använde ränta och hade tillgångar som var otillåtna.

Om vi antar att någon stjäl ett får från en flock och kommer för att ge bort det, då är det förbjudet. Anledningen är att du känner till att fåret inte tillhör personen. Men om personen använder ränta, då syndar personen men det är tillåtet att ta emot dessa tillgångar på ett sätt som den islamiska lagen godkänner. Vi säger till denna kvinna: Du syndar inte om du vallfärdar för pengar som du har fått av någon som använder ränta.

